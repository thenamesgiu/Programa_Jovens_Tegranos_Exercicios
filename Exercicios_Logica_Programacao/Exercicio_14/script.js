function TrocoCliente(){
    ...
    if (PrecoUnit.length == 0 || Compra.length == 0 || Dinheiro.length == 0 || InputValido == false){
        ...
        if (ValorRecebido < Total){
            var Falta = (Total - ValorRecebido).toFixed(2)
            document.getElementById('Resultado').innerHTML="O dinheiro recebido não é suficiente para o pagamento. Faltam R$" + Falta;
